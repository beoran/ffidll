...
func malloc(size int) unsafe.Pointer {
	return (unsafe.Pointer(_C_calloc(_C_size_t(size), _C_size_t(1))))
}
...
func Open(name string) *Library {
	...
	defer cname.free()
	library.ptr = _C_dlopen(cname, RTLD_LOCAL+RTLD_LAZY)
	return library
}
...
func (library *Library) Sym(name string) *Function {
	...
	defer cname.free()
	function.ptr = _C_dlsym(library.ptr, cname)
	return function
}
